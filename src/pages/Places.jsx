  ...
  const filteredPlaces = useMemo(() => {
    return places.filter(place => {
      ...
      const matchesSearch = 
        ...
        place.short_description.toLowerCase().includes(searchText.toLowerCase());
      ...
      <div className="bg-slate-900 py-16 px-4 shadow-lg mb-8 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto relative z-10 text-center md:text-left">
               <h1 className="text-white text-3xl md:text-5xl font-extrabold mb-4 tracking-tight">
                  O'zbekistonning <span className="text-primary-400">Tarixiy Joylari</span>
              </h1>
              <p className="text-slate-300 text-lg max-w-2xl">
                  Mamlakatimizning boy tarixi va madaniy merosi bilan tanishing. O'zingizga qiziq bo'lgan hududni tanlang va sayohatni boshlang.
              </p>
          </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 -mt-10 relative z-20 mb-12">
        <div className="bg-white p-6 rounded-2xl shadow-xl flex flex-col md:flex-row items-center gap-6 border border-slate-100">
          ...
          <div className="flex flex-col gap-2 w-full md:w-1/4">
            <label htmlFor="region-filter" className="font-semibold text-slate-700 flex items-center gap-2">
          ...
          <div className="flex flex-col gap-2 w-full md:w-3/4">
            <label htmlFor="search-input" className="font-semibold text-slate-700 flex items-center gap-2">
            ...
            <div className="relative">
                ...
                />
                 <FontAwesomeIcon icon={faSearch} className="absolute left-3.5 top-1/2 transform -translate-y-1/2 text-slate-400" />
            </div>
          </div>
        </div>
      </div>
      ...
      <main className="max-w-7xl mx-auto px-4">
        {filteredPlaces.length > 0 ? (
          ...
          <div className="text-center py-20 bg-white rounded-2xl shadow-sm border border-slate-100">
            ...
            <h3 className="text-xl font-bold text-slate-800 mb-2">Hech qanday ma'lumot topilmadi</h3>
            <p className="text-slate-500">Qidiruv so'zini o'zgartirib ko'ring yoki boshqa viloyatni tanlang.</p>
            <button 
                onClick={() => {setSearchText(''); setRegionFilter('all');}}
